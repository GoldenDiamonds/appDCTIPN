...
function actualizarSitio1(transaction, results) {
    //initialise the listitems variable
    var listitems = "";
    //get the sitios list holder ul
    var listholder = document.getElementById("listaSitiosTable1");
    ...
    for (i = 0; i < results.rows.length; i++) {
        ...
        */
        listholder.innerHTML += "<tr><td>" + row.Ciudad + "</td>" + 
        "</td></tr>";
    }
...
function salidaSitio1() {
    ...
    if (mydb) {
        ...
        mydb.transaction(function (t) {
            t.executeSql("SELECT * FROM sitios", [], actualizarSitio1);
        });
    } else {
...
function eliminarSitio1(id) {
    //check to ensure the mydb object has been created
    if (mydb) {
        //Get all the sitios from the database with a select statement, set outputSitiosList as the callback function for the executeSql command
        mydb.transaction(function (t) {
            t.executeSql("DELETE FROM sitios WHERE id=?", [id], salidaSitio1);
        });
    } else {
        alert("¡Tú navegador web/browser no soporta WebSQL!");
    }
